...
jQuery(document).ready(function($){
	...
	var menuItemsDelay = ["12","24","9"]; // set the delay values if you want to use random animations. Use only one value if you want the delay to be the same. Use "0" as a single value if you don't want a delay.
	var menuItemDelay = 0;
	...
	function showMenuItems() {
		...
		$('.mainMenuWrapper > li').each(function(){
			...
			$(this).find('.mainMenuItemOverlay').stop(true, true).delay(menuItemDelay).animate({"width": 0}, 100, 'easeInQuart');
			
		});
		
		$('.mainMenuOuterWrapper > .mainMenuButton').stop(true, true).fadeIn(100);
	...
	function hideMenuItems(){
		...
		$('.mainMenuOuterWrapper > .mainMenuButton').stop(true, true).delay(100).fadeOut(100);
	...
	$('.mainMenuButton').click(function(){
		...
		if(mainMenuStatus == 'closed' && mainMenuAnimation == 'complete'){
			...
			$('.mainMenuOuterWrapper').stop(true, true).animate({"left": 0}, 100, 'easeOutQuart', function(){mainMenuStatus = 'open'; mainMenuAnimation = 'complete'; showMenuItems();});
			
		}else if(mainMenuStatus == 'open' && mainMenuAnimation == 'complete'){
			
			mainMenuAnimation = 'incomplete';
			hideMenuItems();
			$('.mainMenuOuterWrapper').stop(true, true).animate({"left": "100%"}, 100, 'easeInQuart', function(){mainMenuStatus = 'closed'; mainMenuAnimation = 'complete'; $('.mainMenuOuterWrapper').removeAttr('style');});
	...
	$('#portfolioMenuWrapper > li > a').click(function(){
		...
		if(filterVal != 'all'){
			...
			$('.portfolioFilterableItemWrapper').stop(true, true).animate({opacity:1}, 100, 'easeOutCubic');
